...
app.post('/internal/dlr_event', async (req, res) => {
  try {
    ...
    if (upd.rowCount && client_id) {
      ...
      // Failure path: bump supplier failures
      if (finalStatus === 'failed' && supplier_id) {
        await pool.query(
          `UPDATE suppliers
             SET consecutive_failures = consecutive_failures + 1,
                 bind_status = CASE WHEN consecutive_failures + 1 >= max_failures THEN 'unbound'::varchar ELSE bind_status END,
                 status      = CASE WHEN consecutive_failures + 1 >= max_failures THEN 'inactive'::varchar ELSE status END
           WHERE id = $1`,
          [supplier_id]
        );
      }
...
app.get('/internal/suppliers/active_outbound', checkInternalToken, async (req, res) => {
  try {
    const r = await pool.query(
      "SELECT id AS supplier_id, smpp_host AS host, smpp_port AS port, smpp_username AS system_id, smpp_password AS password, smpp_version FROM suppliers WHERE connection_type = 'smpp' AND is_inbound = false AND status = 'active'"
    );
...
app.post('/api/bind/test', auth, async (req, res) => {
  try {
    const { host, port, username, password, interface_version, supplier_id, is_inbound } = req.body || {};
...
app.post('/api/sms/send', auth, async (req, res) => {
  try {
    ...
    // 4. Pick a supplier via route_maps. Best match: highest priority route_map
    //    whose mccmnc_pattern LIKE the destination prefix. If no match, fall back
    //    to the first active route_map for this client. If still none, 'unrouted'.
    let supplier = null, routeId = null, routeName = null, supplierConnType = null;
    if (mcc) {
      const r = await pool.query(
        `SELECT rm.route_id, rm.supplier_id, r.route_name, s.supplier_code, s.connection_type
           FROM route_maps rm
      ...
      if (r.rows.length) {
        ...
        supplierConnType = r.rows[0].connection_type;
    ...
    if (!supplier) {
      ...
      if (fb.rows.length) {
        supplier = { id: fb.rows[0].supplier_id, code: fb.rows[0].supplier_code };
        supplierConnType = fb.rows[0].connection_type;
      ...
      if (routeChannel !== 'sms' && allowed.includes(routeChannel) && numValid) {
        ...
        if (!v || !v.valid) {
          ...
          await pool.query(
            ...
            [msgId, client_id, c.client_code, sender_id, destination, message, routeChannel, routeName]
          );
    ...
    // 8. Insert SMS log — full schema columns (supplier, route, MCC cascade).
...
app.post('/api/billing/invoices', auth, roles('super_admin','admin','billing'), async (req, res) => {
  ...
  // Sum SMS for period — respect billing_mode: submit-mode counts all, dlr-mode counts delivered only
  let billingMode = 'dlr';
  if (entity_type === 'client') {
    const cl = await pool.query('SELECT billing_mode FROM clients WHERE id = $1', [entity_id]);
    billingMode = cl.rows[0]?.billing_mode || 'dlr';
  } else if (entity_type === 'supplier') {
    const sl = await pool.query('SELECT billing_mode FROM suppliers WHERE id = $1', [entity_id]);
    billingMode = sl.rows[0]?.billing_mode || 'dlr';
  }
  const statusFilter = billingMode === 'submit' ? "status IN ('submitted','delivered')" : "status='delivered'";
  const smsR = await pool.query(`SELECT COUNT(*) as total_sms, COALESCE(SUM(client_rate*message_parts),0) as total_amount FROM sms_logs WHERE client_id=$1 AND submit_time::date BETWEEN $2 AND $3 AND ${statusFilter}`, [entity_id, period_start, period_end]);
...
app.post('/api/webhooks/dlr/:connector_id', async (req, res) => {
  try {
    ...
    if (externalMsgId) {
      const upd = await pool.query(`UPDATE sms_logs SET dlr_status=$1, dlr_timestamp=NOW(), status=$2, delivery_time=NOW()
        WHERE (smpp_message_id=$3 OR message_id=$3)
        RETURNING client_id, client_rate, message_parts`,
        [dlrStatus, dlrStatus === 'DELIVRD' ? 'delivered' : 'failed', externalMsgId]);
      // Apply DLR-mode billing: only charge on DELIVRD (same as /internal/dlr_event)
      if (upd.rowCount && dlrStatus === 'DELIVRD') {
        const row = upd.rows[0];
        if (row.client_id) {
          const cl = await pool.query('SELECT billing_mode FROM clients WHERE id = $1', [row.client_id]);
          if (cl.rows[0]?.billing_mode === 'dlr') {
            const cost = parseFloat(row.client_rate) * (row.message_parts || 1);
            await pool.query('UPDATE clients SET balance = balance - $1 WHERE id = $2', [cost, row.client_id]);
          }
        }
      }
    }
...
require("./external-api.cjs")(app, pool, auth, SEED_USER_PASSWORDS);
...
async function voiceDlrPollerOnce() {
  try {      const due = await pool.query(
      `SELECT id, call_id, destination, otp_code, language, retry_count, max_retries, client_id, sip_server_id
    ...
    for (const row of due.rows) {
      ...
      try {
        ...
        if (!chosen) {
          await pool.query(`UPDATE voice_call_retry_queue SET status='failed', error_message='no_servers_available', completed_at=NOW() WHERE id=$1`, [row.id]);
          await pool.query(`UPDATE voice_otp_logs SET dial_status='FAIL_NO_SERVER', status='failed', completed_at=NOW() WHERE call_id=$1`, [row.call_id]);
          await pushSyntheticVoiceDlr(row.call_id, 'FAIL_NO_SERVER', row.destination, row.otp_code, row.language, row.client_id);
          continue;
        }
  ...
  try {
    await pool.query(
      ...
      [msgId, client_id, destination, '[voice-otp] ' + language + ': ' + otp_code,
       status === 'CONNECTED' ? 'delivered' : 'failed', status]
    );
...
// ============================================================
// FORCE DLR SCHEDULER — background timers for billing enforcement
// ============================================================
// 1) Client force-DLR (10s tick): when client.force_dlr=true and
//    billing_mode='dlr', after dlr_timeout seconds without a real
//    DLR, force status to delivered and deduct balance.
// 2) Supplier force-DLR 24h fallback (60s tick): for supplier
//    force_dlr=true, if still submitted after 24h, mark as failed
//    and REVERT billing (credit client back, submit-mode only).
let _forceDlrHandle = null;
function startForceDlrScheduler() {
  if (_forceDlrHandle) return;
  // ── Client Force DLR: check every 10s ─────────────────────────
  setInterval(async () => {
    try {
         ...
         WHERE id IN (
           SELECT sl.id FROM sms_logs sl
            ...
            WHERE sl.status = 'submitted'
              ...
              AND c.force_dlr = true
              AND c.billing_mode = 'dlr'
              AND sl.submit_time + COALESCE(c.dlr_timeout, 150) * INTERVAL '1 second' <= NOW()
            LIMIT 100
         )
         RETURNING client_id, client_rate, message_parts`
      );
      ...
      if (rows.rowCount) console.log(`[force-dlr] client force-DLR: ${rows.rowCount} message(s) marked delivered`);
    } catch (e) { console.warn('[force-dlr] client tick failed:', e.message); }
  }, 10000);
  // ── Supplier Force DLR 24h fallback: check every 60s ──────────
  setInterval(async () => {
    try {
      const rows = await pool.query(`
        UPDATE sms_logs
         ...
         WHERE id IN (
           SELECT sl.id FROM sms_logs sl
            ...
            WHERE sl.status = 'submitted'
              ...
              AND s.force_dlr = true
              AND sl.submit_time + INTERVAL '24 hours' <= NOW()
            LIMIT 100
         )
         RETURNING client_id, client_rate, message_parts`
      );
      for (const row of rows.rows) {
        // Only revert billing for submit-mode clients who were charged at submission
        if (row.client_id) {
          const cl2 = await pool.query('SELECT billing_mode FROM clients WHERE id = $1', [row.client_id]);
          if (cl2.rows[0]?.billing_mode === 'submit') {
            const cost = parseFloat(row.client_rate) * (row.message_parts || 1);
            await pool.query('UPDATE clients SET balance = balance + $1 WHERE id = $2', [cost, row.client_id]);
          }
        }
      }
      if (rows.rowCount) console.log(`[force-dlr] supplier 24h fallback: ${rows.rowCount} message(s) marked failed, billing reverted where applicable`);
    } catch (e) { console.warn('[force-dlr] supplier tick failed:', e.message); }
  }, 60000);
